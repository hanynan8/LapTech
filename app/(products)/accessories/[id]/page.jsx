...
export async function generateStaticParams() {
  try {
    const res = await fetch('https://restaurant-back-end.vercel.app/api/data?collection=accessories', {
            ...
            <div className="flex items-center justify-between mb-1 sm:mb-2">
              ...
              {prod.rating && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs sm:text-sm text-gray-600">{prod.rating}</span>
                </div>
              )}
            </div>
          ...
          <div key={key} className="spec-card card-hover">
            ...
            <div className="text-sm sm:text-base text-gray-700">
              {Array.isArray(value) ? (
                ...
                <div className="leading-relaxed text-sm sm:text-base">{String(value)}</div>
              )}
            </div>
          </div>
        ))}
  ...
  return (
    <section className="bg-white rounded-3xl shadow-lg p-4 sm:p-8 mb-8 fade-in">
      <div className="text-center mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">آراء العملاء</h2>
        <div className="flex items-center justify-center gap-2 sm:gap-4 flex-wrap">
          ...
          <span className="text-xl sm:text-2xl font-bold text-gray-900">{reviews.avgRating}</span>
          <span className="text-sm sm:text-base text-gray-600">({reviews.count} مراجعة)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {reviews.items?.slice(0, 6).map((review, index) => (
          <div key={`review-${review.user}-${index}`} className="review-card rounded-xl p-4 sm:p-6 card-hover">
            ...
            <p className="text-gray-700 mb-2 sm:mb-3 leading-relaxed text-sm sm:text-base">{review.comment}</p>
            <div className="text-xs sm:text-sm text-gray-500">{formatDate(review.date)}</div>
          </div>
        ))}
      </div>
    </section>
...
function ProductAdditionalInfo({ product, details }) {
  ...
  return (
    <aside className="bg-gray-50 rounded-2xl p-4 sm:p-6 mt-4 sm:mt-6">
      <h3 className="font-bold text-base sm:text-lg mb-3 sm:mb-4 text-gray-900">معلومات المنتج</h3>
      <div className="space-y-2 sm:space-y-3">
        <div className="flex justify-between items-center">
          <span className="text-gray-600 text-sm sm:text-base">الحالة</span>
          <span className="font-semibold text-green-600 text-sm sm:text-base">متوفر</span>
        </div>
        <div className="flex justify-between items-center">
        ...
        {additionalInfo.map((info, index) => (
          <div key={`info-${index}`} className="flex justify-between items-center">
            <span className="text-gray-600 text-sm sm:text-base">{info.label}</span>
            <span className="font-medium text-gray-900 text-sm sm:text-base">{info.value}</span>
          </div>
        ))}
      </div>
    </aside>
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-8">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-4 sm:gap-8 p-4 sm:p-8">
                ...
                {Array.isArray(details.additionalImages) && details.additionalImages.length > 0 && (
                  <div className="grid grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-0">
                    {[product.image, ...details.additionalImages].slice(0, 4).map((src, i) => (
                      <a key={`gallery-thumb-${i}`} href={`#img-${i}`} className="bg-gray-50 rounded-lg sm:rounded-xl p-1 sm:p-2 card-hover relative">
                        <img 
                          src={src} 
            ...
            <div className="xl:col-span-3 space-y-4 sm:space-y-6">
              <div>
                ...
                <h1 className="text-2xl sm:text-4xl font-bold text-gray-900 mb-3 sm:mb-4 leading-tight">{product.name}</h1>
              ...
              <div className="quick-specs-grid">
                {Object.entries(specs).slice(0, 6).map(([key, value], i) => (
                  <div key={`quick-spec-${i}`} className={`p-2 sm:p-4 rounded-xl text-center ${
              ...
              {details.description && (
                <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                  ...
                  <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{details.description}</p>
                </div>
              )}
              ...
              {(specs.features || details.features) && (
                <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">الميزات الرئيسية</h3>
                  <div className="grid grid-cols-1 gap-2 sm:gap-3">
                    <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white rounded-lg sm:rounded-xl">
                      <div className="w-2 h-2 bg-purple-500 rounded-full flex-shrink-0"></div>
                      <span className="text-gray-700 font-medium text-sm sm:text-base">{specs.features || details.features}</span>
                    </div>
                  </div>
                </div>
              )}
        ...
        {details.inTheBox && (
          <section className="bg-white rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-8 mb-6 sm:mb-8 fade-in">
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">محتويات الصندوق</h3>
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {details.inTheBox.split('،').map((item, index) => (
                <span key={`accessory-${index}`} className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-purple-100 to-blue-100 text-purple-700 rounded-full text-xs sm:text-sm font-medium border border-purple-200">
                  {item.trim()}
                </span>
              ))}
            </div>
        ...
        {(details.healthFeatures || details.chargingSpeed || details.waterproof) && (
          <section className="bg-white rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-8 mb-6 sm:mb-8 fade-in">
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 text-center">ميزات متقدمة</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {details.healthFeatures && (
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center">
                  ...
                  <h4 className="font-bold text-base sm:text-lg text-green-700 mb-1 sm:mb-2">الميزات الصحية</h4>
                  <p className="text-green-600 text-xs sm:text-sm">{details.healthFeatures}</p>
                </div>
              )}
              {details.chargingSpeed && (
                <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center">
                  <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">⚡</div>
                  <h4 className="font-bold text-base sm:text-lg text-blue-700 mb-1 sm:mb-2">الشحن السريع</h4>
                  <p className="text-blue-600 text-xs sm:text-sm">{details.chargingSpeed}</p>
                </div>
              )}
              {details.waterproof && (
                <div className="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center">
                  <div className="text-2xl sm:text-3xl mb-2 sm:mb-3">💧</div>
                  <h4 className="font-bold text-base sm:text-lg text-cyan-700 mb-1 sm:mb-2">مقاومة الماء</h4>
                  <p className="text-cyan-600 text-xs sm:text-sm">{details.waterproof}</p>
                </div>
              )}
            </div>
          </section>
        ...
        {(details.technology || details.design || details.efficiency) && (
          <section className="bg-gradient-to-br from-gray-50 to-white rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-8 mb-6 sm:mb-8 fade-in">
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 text-center">معلومات تقنية إضافية</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
              {details.technology && (
                <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{details.technology}</p>
                </div>
              )}
              {details.design && (
                <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{details.design}</p>
                </div>
              )}
              {details.efficiency && (
                <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{details.efficiency}</p>
                </div>
              )}
              {details.cooling && (
                <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100">
                  ...
                  <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{details.cooling}</p>
                </div>
              )}
            </div>
          </section>
        ...
        {(details.bluetoothRange || details.pollingRate || details.frequency) && (
          <section className="bg-white rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-8 mb-6 sm:mb-8 fade-in">
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 text-center">مواصفات الاتصال والأداء</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
              {details.bluetoothRange && (
                <div className="text-center p-4 sm:p-6 bg-blue-50 rounded-xl sm:rounded-2xl">
                  <div className="text-xl sm:text-2xl font-bold text-blue-600 mb-1 sm:mb-2">{details.bluetoothRange}</div>
                  <div className="text-xs sm:text-sm text-blue-700">مدى البلوتوث</div>
                </div>
              )}
              {details.pollingRate && (
                <div className="text-center p-4 sm:p-6 bg-green-50 rounded-xl sm:rounded-2xl">
                  <div className="text-xl sm:text-2xl font-bold text-green-600 mb-1 sm:mb-2">{details.pollingRate}</div>
                  <div className="text-xs sm:text-sm text-green-700">معدل الاستجابة</div>
                </div>
              )}
              {details.frequency && (
                <div className="text-center p-4 sm:p-6 bg-purple-50 rounded-xl sm:rounded-2xl">
                  <div className="text-xl sm:text-2xl font-bold text-purple-600 mb-1 sm:mb-2">{details.frequency}</div>
                  <div className="text-xs sm:text-sm text-purple-700">نطاق التردد</div>
                </div>
              )}
            </div>
          </section>
        ...
        <section className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-8 text-white text-center fade-in">
          <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">لماذا تختار {product.name}؟</h3>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mt-4 sm:mt-8">
            <div className="bg-white/10 backdrop-blur rounded-xl sm:rounded-2xl p-3 sm:p-4">
              ...
              <div className="font-bold mb-1 text-sm sm:text-base">جودة عالية</div>
              <div className="text-xs sm:text-sm opacity-90">منتج أصلي وموثوق</div>
            </div>
            <div className="bg-white/10 backdrop-blur rounded-xl sm:rounded-2xl p-3 sm:p-4">
              <div className="text-lg sm:text-2xl mb-1 sm:mb-2">🚚</div>
              <div className="font-bold mb-1 text-sm sm:text-base">توصيل سريع</div>
              <div className="text-xs sm:text-sm opacity-90">وصول خلال 24-48 ساعة</div>
            </div>
            <div className="bg-white/10 backdrop-blur rounded-xl sm:rounded-2xl p-3 sm:p-4">
              <div className="text-lg sm:text-2xl mb-1 sm:mb-2">🛡️</div>
              <div className="font-bold mb-1 text-sm sm:text-base">ضمان موثوق</div>
              <div className="text-xs sm:text-sm opacity-90">{details.warranty || 'ضمان الشركة'}</div>
            </div>
            <div className="bg-white/10 backdrop-blur rounded-xl sm:rounded-2xl p-3 sm:p-4">
              <div className="text-lg sm:text-2xl mb-1 sm:mb-2">💎</div>
              <div className="font-bold mb-1 text-sm sm:text-base">خدمة متميزة</div>
