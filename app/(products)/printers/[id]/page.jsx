...
export default async function ProductDetailsPage({ params }) {
  ...
  async function fetchPrintersData() {
    ...
    // البيانات داخل مصفوفة، نحتاج للعنصر الأول
  ...
  async function fetchProductById(id) {
    try {
      ...
      if (!printersData || !printersData.products) return null;
      
      // البحث عن المنتج بالـ id
      const product = printersData.products.find(p => p.id === id);
      return product || null;
    } catch (error) {
  ...
  async function fetchRelatedProducts(category, currentProductId, limit = 8) {
    try {
      const printersData = await fetchPrintersData();
      if (!printersData || !printersData.products) return [];
      
      // تصفية المنتجات من نفس الفئة باستثناء المنتج الحالي
      const relatedProducts = printersData.products
        .filter(p => p.category === category && p.id !== currentProductId && p.isActive)
        .slice(0, limit);
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {product.availability && (
                    <div className="text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded-full inline-block w-fit">
                      {product.availability}
                    </div>
                  )}
                </div>
          ...
          <div className="lg:hidden p-6">
            ...
            <div className="bg-green-50 p-5 rounded-2xl border border-green-200 mb-6">
              ...
              <div className="text-green-700">
                <p>• {product.availability || 'متوفر للشحن'}</p>
                <p>• شحن مجاني للطلبات فوق 5000 ج.م</p>
                <p>• إرجاع مجاني خلال 14 يوم</p>
                {product.warrantyMonths && <p>• الضمان: {product.warrantyMonths} شهر</p>}
              </div>
            </div>
                  ...
                  >
                    ...
                    {rp.performanceScore && (
                      <div className="text-xs text-gray-500 mt-1">
                        الأداء: {rp.performanceScore}/100
                      </div>
                    )}
                  </Link>
                ))}
