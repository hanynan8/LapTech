...
import { Search, Filter, Star, ShoppingCart, Heart, Eye, ArrowRight, HardDrive, Cpu, Database, Smartphone, CreditCard, Usb, Zap, Wifi, Monitor, MemoryStick, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ProductCard = React.memo(({ product, favorites, toggleFavorite, index }) => {
  ...
  return (
    ...
    >
      <Link href={`/storage-devices/${product.id}`} className="group block">
...
const StorageClient = ({ initialData }) => {
        ...
        if (searchTerm.trim()) {
          ...
          filtered = filtered.filter(product =>
            ...
            (product.specs && product.specs.capacity && 
             product.specs.capacity.toLowerCase().includes(term)) ||
            (product.specs && product.specs.type && 
             product.specs.type.toLowerCase().includes(term))
          );
        }
          ...
          case 'capacity':
            filtered.sort((a, b) => {
              const getCapacityValue = (capacity) => {
                ...
                const match = capacity.toString().match(/(\d+\.?\d*)/);
                if (!match) return 0;
                const value = parseFloat(match[1]);
                if (capacity.toLowerCase().includes('tb')) return value * 1000;
                if (capacity.toLowerCase().includes('gb')) return value;
                if (capacity.toLowerCase().includes('mb')) return value / 1000;
                return value;
              };
              return getCapacityValue(b.specs?.capacity) - getCapacityValue(a.specs?.capacity);
            });
            break;
          case 'newest':
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            ...
            <div className="relative flex-1 max-w-2xl">
              ...
              <input
                type="text"
                placeholder="ابحث عن وسيط التخزين المناسب..."
                value={searchQuery}
      ...
      <section className="py-20 bg-gradient-to-r from-purple-600 to-blue-600 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center text-white">
          ...
          <p className="text-xl mb-8 opacity-90">
            تواصل معنا وسنساعدك في العثور على وسيط التخزين المثالي
          </p>
