...
const StorageMediaPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // البحث عن بيانات وسائط التخزين
      let storageData = null;

      if (
        apiData['storage-devices'] &&
        Array.isArray(apiData['storage-devices']) &&
        apiData['storage-devices'].length > 0
      ) {
        storageData = apiData['storage-devices'][0];
      } else {
        throw new Error('لم يتم العثور على بيانات وسائط التخزين في الاستجابة');
      }
      ...
      const processedData = {
        ...
        pageSubtitle:
          storageData.pageSubtitle ||
          'اكتشف أفضل حلول التخزين من أقراص صلبة، SSD، فلاش ميموري وأكثر',
    ...
    switch (sortBy) {
      ...
      case 'capacity':
        filtered.sort((a, b) => {
          ...
          return (
            getCapacityValue(b.specs?.capacity) -
            getCapacityValue(a.specs?.capacity)
          );
        });
        break;
      default:
          ...
          {filteredProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {filteredProducts.map((product) => (
                <Link
                  href={`/storage-devices/${product.id}`}
                  key={product.id}
...
export default StorageMediaPage;
