...
const LocationPage = () => {
  ...
  useEffect(() => {
    const fetchLocationData = async () => {
      try {
        ...
        // تحقق من هيكل البيانات المختلفة المحتملة
        let locationInfo = null;
        
        if (data.location && Array.isArray(data.location) && data.location.length > 0) {
          // إذا كانت location مصفوفة
          if (data.location[0].location && Array.isArray(data.location[0].location)) {
            locationInfo = data.location[0].location[0];
          } else {
            locationInfo = data.location[0];
          }
        } else if (data.location && !Array.isArray(data.location)) {
          // إذا كانت location كائن مباشر
          locationInfo = data.location;
        } else {
          // تجربة البحث في الاستجابة عن أي بيانات موقع
          console.log('Searching for location data in response structure...');
          locationInfo = findLocationData(data);
        }
        
        console.log('Extracted location data:', locationInfo);
  ...
  // دالة للبحث عن بيانات الموقع في هيكل معقد
  const findLocationData = (obj) => {
    if (!obj || typeof obj !== 'object') return null;
    
    // البحث عن pageTitle كمؤشر لبيانات الموقع
    if (obj.pageTitle) return obj;
    
    // البحث العميق في الكائن
    for (let key in obj) {
      if (obj[key] && typeof obj[key] === 'object') {
        const found = findLocationData(obj[key]);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center" dir="rtl">
        <div className="text-center bg-white p-8 rounded-2xl shadow-lg max-w-md">
          ...
          <p className="text-xs text-gray-400 mt-4">
            API URL: https://restaurant-back-end.vercel.app/api/data
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50" dir="rtl">
                  ...
                  {locationData.mainLocation?.details?.map((detail, index) => {
                    ...
                    return (
                      <div key={index} className="flex items-start">
                        <div className={`bg-gradient-to-br ${detail.color} w-12 h-12 rounded-xl flex items-center justify-center ml-4 flex-shrink-0`}>
                ...
                <div className="bg-white rounded-3xl p-8 shadow-lg">
                  ...
                  <div className="bg-gradient-to-br from-gray-200 to-gray-300 rounded-2xl h-80 flex items-center justify-center mb-6">
                    <div className="text-center text-gray-600">
                      ...
                      <p className="text-xl font-bold">{locationData.mapSection.placeholder?.title}</p>
                      <p className="text-sm">{locationData.mapSection.placeholder?.address}</p>
                    </div>
                  </div>
                  <div className="flex space-x-4 space-x-reverse">
                    ...
                    <button className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 transition-colors">
                      {locationData.mapSection.buttons?.share}
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {locationData.branches.locations?.map((branch, index) => (
                <div key={index} className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300">
                  <div className={`bg-gradient-to-br ${branch.color} w-16 h-16 rounded-2xl flex items-center justify-center mb-6`}>
                  ...
                  <button className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white py-3 rounded-xl font-bold hover:shadow-lg transform hover:scale-105 transition-all duration-300">
                    {locationData.branches.viewButton}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
            ...
            <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-6 sm:space-x-reverse">
              <button className="bg-white text-purple-600 px-12 py-4 rounded-full text-xl font-bold hover:shadow-2xl transform hover:scale-105 transition-all duration-300">
                {locationData.visitSection.buttons?.visit}
              ...
              <button className="bg-white/20 backdrop-blur-sm text-white px-12 py-4 rounded-full text-xl font-bold hover:bg-white/30 transition-all duration-300">
                {locationData.visitSection.buttons?.call}
      ...
      {/* Debug Info (للتطوير فقط) */}
      <div className="hidden">
        <details className="bg-gray-100 p-4 m-4 rounded">
