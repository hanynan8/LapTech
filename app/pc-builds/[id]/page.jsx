...
export default async function ProductDetailsPage({ params }) {
  ...
  async function fetchByCategory(category, limit = 12) {
    try {
      const res = await fetch(
        `https://restaurant-back-end.vercel.app/api/data?collection=pc-build&category=${encodeURIComponent(
          category
        )}&limit=${limit}`,
        { next: { revalidate: 60 } }
      );
  ...
  // استخراج البيانات بشكل صحيح من هيكل بيانات PC Build
  const specs = product.specs || {};
  const details = product.details || {};
  const components = product.components || [];
  const benchmarks = product.benchmarks || {};
  const isAvailable =
    details.stockStatus === 'In Stock' || details.stockStatus === 'Limited';
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="lg:hidden mb-6 bg-white p-6 rounded-2xl shadow-sm">
          ...
          <p className="text-lg text-gray-600 mb-4">
            {details.manufacturer || product.category}
          </p>
              ...
              {details && Object.keys(details).length > 0 && (
                <div className="bg-indigo-50 p-6 rounded-2xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {details.estimatedPowerW && (
                      <div className="bg-white p-4 rounded-xl shadow-xs border border-gray-100">
                        <div className="text-sm font-medium text-gray-500 mb-1">
                          استهلاك الطاقة المقدر
                        </div>
                        <div className="font-semibold text-indigo-800">
                          {details.estimatedPowerW} واط
                        </div>
                      </div>
                    )}
                    ...
                    {details.supportsMultiGPU !== undefined && (
                      <div className="bg-white p-4 rounded-xl shadow-xs border border-gray-100">
                        <div className="text-sm font-medium text-gray-500 mb-1">
                          دعم كروت رسوميات متعددة
                        </div>
                        <div className="font-semibold text-indigo-800">
                          {details.supportsMultiGPU ? 'مدعوم' : 'غير مدعوم'}
                        </div>
                      </div>
                    )}
