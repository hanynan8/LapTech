...
export async function getCollectionData(collectionName, { id } = {}) {
  try {
    ...
    if (id) {
      ...
      // fetched via collection=carts&id=<_id>).
      //
      // ملاحظة مهمة: mongoose.Types.ObjectId.isValid() عندها خلل معروف -
      // بترجع true لأي قيمة من نوع number (زي 2 أو 102) حتى لو مش
      // ObjectId حقيقي، لأن ObjectId ممكن يتبني من رقم كـ timestamp.
      // ده كان بيخلي أي منتج ID رقمي (زي details.relatedProducts اللي
      // بتكون [102, 103] أرقام فعلية مش نصوص) يدخل على findById() ويفشل
      // بـ "Cast to ObjectId failed". الفحص هنا بيتطلب نص (string) فعلي
      // بطول 24 حرف hex - ده الشكل الوحيد لـ ObjectId حقيقي جاي من رابط.
